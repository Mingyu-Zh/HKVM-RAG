...
To test whether the WHG-KV advantage reflects a weak pairwise reference rather than a genuine key-space effect, we implement two topological enhancements from HippoRAG~2 \cite{hipporag2} within the same fixed substrate: passage nodes (adding passages as graph vertices connected to their contained entity nodes) and query-to-triple linking (seeding PPR from query--triple token overlap in addition to entity matching). These enhancements improve the pairwise graph topology without changing the evidence tuple cache, candidate passages, or retrieval algorithm (PPR). Table~\ref{tab:supp_enhanced_kg} reports the four variants across all three benchmarks.
...
Passage nodes alone produce marginal changes from the original pairwise baselines ($|\Delta\mathrm{F1}|<0.7$ across all datasets). Query-to-triple linking helps on 2WikiMultiHopQA (+2.1 F1) but degrades performance on MuSiQue ($-$1.3 to $-$1.5 F1), where token-overlap matching against complex compositional questions introduces noise. The strongest enhanced pairwise variant (Weighted-KG-PPR + PN + QT, 78.041 F1 on 2Wiki) remains below WHG-KV (79.299). On MuSiQue, no enhancement narrows the gap below $+3.2$ F1. These results indicate that the hypergraph key-space advantage is not an artifact of a weak pairwise reference: even the best available pairwise topology improvements do not replicate the evidence-organization gain of answer-path hyperedges under the shared extraction substrate.
